...
public class SqlTimeIncrementProvider implements SqlTimeCorruptedProvider {
  ...
  @JsonCreator
  public SqlTimeIncrementProvider(
      @JsonProperty("start") String start,
      @JsonProperty("step") Long step,
      @JsonProperty("unit") String unit
  ) {
    this.start = start == null ? LocalTime.now(Clock.systemUTC()) : LocalTime.parse(start);
  ...
  public Time value(FieldContext fieldContext) {
    return Time.valueOf(start.plus(fieldContext.rowIndex() * step, unit));
  }
}
