...
public record Column(
    ...
    @JsonProperty("corrupted_rate") @JsonAlias({"corruptedRate",
        "corrupted-rate"}) Float corruptedRate,
  ...
  public Column {
    ...
    assert nullRate == null || (nullRate >= 0 && nullRate <= 1) :
        "Column nullRate must be between 0 and 1";
    assert corruptedRate == null || (corruptedRate >= 0 && corruptedRate <= 1) :
        "Column corruptedRate must be between 0 and 1";
