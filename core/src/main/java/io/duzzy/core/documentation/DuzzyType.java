...
public enum DuzzyType {
  DUZZY_CONFIG(
      "Duzzy Config",
      "Duzzy Config enables schema enrichment by specifying column provider, "
          + "sink or serializer"
  ),
  DUZZY_SCHEMA(
      "Duzzy Schema",
      "Duzzy schema is the default input schema "
          + "in which you can specify whatever you want into the output"
  ),
  PARSER(
      "Parser",
      "Parser parses the input schema and produces a DuzzyContext "
          + "by combining input schema and duzzy config. "
  ...
  SINK(
      ...
      "Sink component writes data into the configured output.  \n"
          ...
          + "sink delegates data serialization to the specified serializer."
  ),
  SERIALIZER(
      "Serializer",
