...
public class DoubleConstantProviderTest {
  ...
  void parsedFromYaml() throws IOException {
    ...
    assertThat(provider.value(SEEDED_ONE_FIELD_CONTEXT.get())).isEqualTo(1d);
  }
  ...
  void computeValueIsIdempotent() {
    final Double value = new DoubleConstantProvider(3d).value(SEEDED_ONE_FIELD_CONTEXT.get());
    assertThat(value).isEqualTo(3d);
  }

  @Test
  void computeValueIsConstant() {
    final Double value = new DoubleConstantProvider(3d).value(SEEDED_FIVE_FIELD_CONTEXT.get());
    assertThat(value).isEqualTo(3d);
  }
  ...
  void corruptedValueIsIdempotent() {
    final Double value =
        new DoubleConstantProvider(3d).corruptedValue(SEEDED_ONE_FIELD_CONTEXT.get());
