...
public class DuzzyProcessingTest {
  ...
  void shouldRunProcessingWithEmptyRowKey() throws Exception {
    ...
    final DataItems expected = new DataItems(
        8213365047359667313L,
        Optional.empty(),
        List.of(new DataItem("testField", Type.STRING, "value"))
    );
  ...
  void shouldRunProcessing() throws Exception {
    ...
    final DataItems expected = new DataItems(
        8213365047359667313L,
        Optional.of("myKey"),
        List.of(new DataItem("myField", Type.STRING, "myValue"))
    );
