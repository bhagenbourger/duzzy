...
public class DuzzyDoc {
  ...
  private static Predicate<DocumentationRecord> predicate(
    ...
    if (duzzyType != null) {
      predicate = predicate.and(d -> d.duzzyType() != null && d.duzzyType() == duzzyType);
    }
    if (duzzyIdentifier != null && !duzzyIdentifier.isEmpty()) {
      predicate = predicate.and(
          d -> d.identifier() != null
              && d.identifier().toLowerCase(ROOT).contains(duzzyIdentifier.toLowerCase(ROOT))
      );
    }
    if (module != null && !module.isEmpty()) {
      predicate = predicate.and(
          d -> d.module() != null && d.module().toLowerCase(ROOT).contains(module.toLowerCase(ROOT))
      );
    }
    if (nativeSupport) {
