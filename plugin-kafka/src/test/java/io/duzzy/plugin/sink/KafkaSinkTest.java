...
public class KafkaSinkTest {
    ...
    void writeXmlMessage() throws IOException {
        ...
        final ConsumerRecords<String, byte[]> records = consumer.poll(Duration.ofSeconds(10));
        final Iterator<ConsumerRecord<String, byte[]>> iterator = records.iterator();
        assertThat(new String(iterator.next().value())).isEqualTo("<?xml version='1.0' encoding='UTF-8'?><rows><row><c1>1</c1><c2>one</c2></row></rows>");
        assertThat(new String(iterator.next().value())).isEqualTo("<?xml version='1.0' encoding='UTF-8'?><rows><row><c1>2</c1><c2>two</c2></row></rows>");
    }
